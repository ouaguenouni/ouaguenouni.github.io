...
- `elicitation.css`, `sampling-overview.css`, and `elicitation.js` — cards 13–15: recap, anonymity/cost principles, the 22-build Borda-to-protocol walkthrough, and the preserved 14-build Hoeffding development in its downward branch.
...
- The numeric endpoint fixes m=10 and illustrates k=5, retaining pairwise tolerance epsilon=.05 and simultaneous failure probability delta=.05. Q=45 and n=ceil(200 ln(40Q))=1500. Full rankings need 1500 people; balanced single-pair queries need 67500. Uniform random 5-rankings have mean-coverage benchmark 6750, not a guaranteed population or an expected all-pairs stopping time. Stop when every pair has n respondents, using its first n observations. Within-person pair outcomes need not be independent. Raw Borda error is at most (m−1)epsilon; this is not an optimal Borda-only estimator claim.
